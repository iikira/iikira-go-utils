...
import (
	...
	"github.com/iikira/iikira-go-utils/utils/prealloc"
...
func (der *Downloader) Execute() error {
	...
	utils.Trigger(der.onExecuteEvent)
	...
	err = der.monitor.Err()
	if err == nil { // 成功
		utils.Trigger(der.onSuccessEvent)
		if !single {
	...
	// 执行结束
	utils.Trigger(der.onFinishEvent)
	return err
}
...
func (der *Downloader) Pause() {
	...
	utils.Trigger(der.onPauseEvent)
	der.monitor.Pause()
}
...
func (der *Downloader) Resume() {
	...
	utils.Trigger(der.onResumeEvent)
	der.monitor.Resume()
}
...
func (der *Downloader) Cancel() {
	...
	utils.Trigger(der.onCancelEvent)
	utils.Trigger(der.monitorCancelFunc)
}
